...
// About lab: This lab handles filesystem commands for client to server
// About file: This file in particular handles the client side.
      ...
      int r = 0;
        switch (findCmd(name[0])) {
            ...
            case 7:
              ...
              int fileSize = 0, bytesRead = 0, gd, found = 0;
              char cbuf[BLKSIZE];
              read(sock, &found, sizeof(found)); // file found server side?
              if (found == 1) {
                read(sock, &fileSize, MAX); // get filesize
                printf("Filesize = %d \n", fileSize);
                if ((gd = open(name[1], O_WRONLY|O_CREAT, 0644)) < 0) {
                  printf("File not opened\n");
                  write(sock, &found, sizeof(found)); // file not created, don't send info
                } else { // begin copying
                  found = 2;
                  write(sock, &found, sizeof(found)); // file created, send info
                  while (fileSize > bytesRead) {// read whole file
                    n = read(sock, cbuf, MAX); // read from sock
                    write(gd, cbuf, n); // write to file
                    bytesRead += n;
                  }
                  close (gd);
            ...
            case 8:
              ...
              if (((fd = (open(name[1], O_RDONLY))) < 0)) { 
                ...
                read(sock, &fcreated, MAX);
                if (fcreated == 1) {
                  stat(name[1], &st);
                  int size = st.st_size;
                  n = write(sock, &size, sizeof(size)); // send size
                  printf("client: put %s of %d bytes\n", name[1], size);
                  
                  while (n = read(fd, sbuf, MAX)) {
                    write(sock, sbuf, n); // send info
                  }
                  close(fd);
                  printf("Transfer Complete\n");
                } else { printf("File creation failed\n"); }
              }
            break; // put
